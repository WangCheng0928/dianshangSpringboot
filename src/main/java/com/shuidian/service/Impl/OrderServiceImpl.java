...
import com.shuidian.VO.OrderItemVo;
import com.shuidian.VO.OrderProductVo;
import com.shuidian.VO.OrderVo;
import com.shuidian.VO.ShippingVo;
import com.shuidian.domain.*;
import com.shuidian.enums.Const;
import com.shuidian.repository.*;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId){
        //从购物车获取数据
        List<Cart> cartList = cartRepository.findCartCheckedStatusByUserId(userId);
        //计算这个订单的总价
        ...
        for (OrderItem orderItem : orderItemList){
            ...
            orderItemRepository.save(orderItem);//这里可以试下批量插入
        }
        //生成成功，我们要减少产品库存
    ...
    private OrderVo assembleOrderVo(Order order, List<OrderItem> orderItemList){
        ...
        orderVo.setImageHost(Const.FTP_PREFIX_VALUE);
    ...
    public ServerResponse<String> cancel(Integer userId, Long orderNo){
        ...
        Order returnOrder = orderRepository.save(updateOrder);
        if (returnOrder != null){
    ...
    public ServerResponse getOrderCartProduct(Integer userId){
        ...
        orderProductVo.setImageHost(Const.FTP_PREFIX_VALUE);
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList, Integer userId){
        ...
        for (Order order : orderList){
            List<OrderItem> orderItemList = Lists.newArrayList();
            if (userId == null){
                // todo 管理员查询时，不需要userId
                orderItemList = orderItemRepository.findByOrderNo(order.getOrderNo());
            }else {
                orderItemList = orderItemRepository.findByUserIdAndOrderNo(userId, order.getOrderNo());
            }
            OrderVo orderVo = assembleOrderVo(order, orderItemList);
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path){
        ...
        String subject = new StringBuilder().append("小猪佩奇门店扫码支付，订单号:").append(outTradeNo).toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                logger.info("qrPath:" + qrPath);
                String qrUrl = Const.FTP_PREFIX_VALUE + targetFile.getName();
    ...
    public ServerResponse alipayCallback(Map<String, String> params){
        ...
        if (order == null){
            return ServerResponse.createByErrorMessage("非小猪佩奇门店的订单，回调忽略");
        }
